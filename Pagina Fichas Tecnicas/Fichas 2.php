...
//	error_reporting(E_ALL);
	...
	if(isset($Anio)){
		if(!(isset($Modificar))){
			...
			$SQLFicha="SELECT * from FichaTecnica where IdAutomovil='$Modelo' and Anio='$Anio' Order by Anio";
			$datosFicha = $conex->ejecutarQuerySQL($SQLFicha);
			if ($Modificar=="si"){
				$SQLGrid7="SELECT c.IdFicha, e.Modelo, f.Marca, c.Anio, a.IdCaracteristica, a.Descripcion as Caracteristica, a.Tipo, b.IdFamilia, b.Descripcion as Familia ,d.Valor";
				$SQLGrid7.=" FROM catCaracteristicas a, catFamilias b ,FichaTecnica c, detFichaTecnica d, catAutomovil e, catMarcas f";
				$SQLGrid7.=" WHERE a.IdFamilia=b.IdFamilia and a.IdCaracteristica=d.IdCaracteristica and c.IdFicha=d.IdFicha and c.IdAutomovil=e.IdAutomovil and e.IdMarca=f.IdMarca";
				$SQLGrid7.=" and a.Activo=1 and b.Activo=1 and c.Activo=1 and e.Activo=1 and f.Activo=1 and c.IdFicha='".$datosFicha[0]["IdFicha"]."' order by c.IdFicha";
				$Grid7 = $conex->ejecutarQuerySQL($SQLGrid7);
				// Modificar
				if(count($Grid7)!=0){
					if(count($imprimeValor)>0){
					  for($i=0;$i<count($imprimeValor);$i++){
						$valores=split("_",$gridValor[$i]);
						$ficha=$valores[0];
						$caracteristica=$valores[1];
						$value=$valores[2];
						$modificar=$imprimeValor[$i];
						$SQL="UPDATE detFichaTecnica";
						$SQL.=" SET Valor=$modificar";
						$SQL.=" WHERE idFicha=$ficha and idCaracteristica='$caracteristica'";					
						$update = $conex->ejecutarQuerySQL($SQL);
					  }
					}
				}
			}else{
		...
		if(!(isset($Grabar))){
			...
			if ($Grabar=="si"){
				...
				if(count($Grid)!=0){
					...
					$SQLGrid2="SELECT c.IdFicha, e.Modelo, f.Marca, c.Anio";
					$SQLGrid2.=" FROM FichaTecnica c, catAutomovil e, catMarcas f";
					$SQLGrid2.=" WHERE c.IdAutomovil=e.IdAutomovil and e.IdMarca=f.IdMarca";
					$SQLGrid2.=" and e.Activo=1 and f.Activo=1 and c.IdFicha='".$datosFicha[0]["IdFicha"]."' order by c.IdFicha";
...
*/
						for($i=0;$i<count($catCaracteristicas);$i++){
							$SQL="INSERT into detFichaTecnica(IdFicha,IdCaracteristica,Valor) values ('".$Ficha[0]["IdFicha"]."','".$catCaracteristicas[$i]["IdCaracteristica"]."',$Valor)";
							$detFichaTecnica=$conex->ejecutarQuerySQL($SQL);
						}
					}else{
						...
						for($i=0;$i<count($catCaracteristicas);$i++){
							$SQL="INSERT into detFichaTecnica(IdFicha,IdCaracteristica,Valor) values ('".$datosFicha[0]["IdFicha"]."','".$catCaracteristicas[$i]["IdCaracteristica"]."',$Valor)";
							$detFichaTecnica=$conex->ejecutarQuerySQL($SQL);
						}
/*
			...
			if(!(isset($Eliminar))){
				...
				if ($Eliminar=="si"){
					...
					if(count($Grid5)!=0){
						...
						$SQLGrid6="SELECT c.IdFicha, e.Modelo, f.Marca, c.Anio";
						$SQLGrid6.=" FROM FichaTecnica c, catAutomovil e, catMarcas f";
						$SQLGrid6.=" WHERE c.IdAutomovil=e.IdAutomovil and e.IdMarca=f.IdMarca";
						$SQLGrid6.=" and e.Activo=1 and c.Activo=1 and f.Activo=1 and c.IdFicha='".$datosFicha[0]["IdFicha"]."' order by c.IdFicha";
						$Grid6 = $conex->ejecutarQuerySQL($SQLGrid6);

						$SQL="SELECT * FROM detFichaTecnica WHERE IdFicha='".$datosFicha[0]["IdFicha"]."'";
						$checa1 = $conex->ejecutarQuerySQL($SQL);

						$SQL="SELECT * FROM colores WHERE IdFicha='".$datosFicha[0]["IdFicha"]."'";
						$checa2 = $conex->ejecutarQuerySQL($SQL);
	
						if( (count($Grid6)!=0) && (count($checa1)==0) && (count($checa2)==0) ){
							$SQL="UPDATE FichaTecnica";
							$SQL.=" SET Activo=0";					
							$SQL.=" WHERE idFicha='".$datosFicha[0]["IdFicha"]."'";					
							...
							<td>
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										...
										<td align="left">
											<input type="button" name="btnModificar" value="   Modificar   " onClick="validarModificar()">
										</td>
									</tr>
								</table>
							</td>
			...
			<tr>	
				<td colspan="2">
					<table width="90%" border="1" cellpadding="0" cellspacing="0">
						<tr> 
							<td colspan="7" bgcolor="yellow"><div align="center">Datos Capturados</div></td>
						</tr>
						<tr> 
							<td width="6%" bgcolor="#669966">
								<div align="center">IdFicha</div></td>
							<td width="33%" bgcolor="#66CC12">
								<div align="center">Caracteristica</div></td>
							<td width="22%" bgcolor="#669966">
								<div align="center">Familia</div></td>
							<td width="11%" bgcolor="#66CC12">
								<div align="center">Tipo</div></td>
							<td width="11%" bgcolor="#669966">
								<div align="center">Valor Actual</div></td>
							<td width="11%" bgcolor="#66CC12">
								<div align="center">Nuevo Valor</div></td>
							<td width="6%" bgcolor="#669966">
						...
						<tr bgcolor="<? echo $color2 ?>"> 
							...
							<td align="center">
								<input type="hidden" name="gridValor[]" value="<?=$Grid3[$i]["IdFicha"]; ?>_<?=$Grid3[$i]["IdCaracteristica"]; ?>_<?=$Grid3[$i]["Valor"]; ?>">
								<input type="text" name="imprimeValor[]" size="12" value="<?=$Grid3[$i]["Valor"]; ?>" onBlur="checarNumero(this)">
							</td>
