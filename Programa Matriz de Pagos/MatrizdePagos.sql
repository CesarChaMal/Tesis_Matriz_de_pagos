...
-- Create Procedure CargaMatriz @NombreTabla VarChar(20) as
 ...
 set @NombreTabla = 'Matriz070807bis'
...
Select m.Marca, a.Modelo, c.Descripcion, c.idCaracteristica,  --t.Promedio, sum(df.valor),
	Proporcion = (t.Promedio - sum(df.valor)) / t.Promedio --1-(sum(df.valor)/t.Total)
...
-- Select t.*, P.Peso, Factor = (t.Proporcion * p.Peso) --* -1
Select t.*, P.Peso as 'Factor', Peso = (t.Proporcion * p.Peso) --* -1
into ##tmpMatriz
...
Select t.*, P.Peso as 'Factor', Peso = t.Proporcion * p.Peso
